...
test.describe('i18n', () => {
  test('page renders with English text by default', async ({ adminPage }) => {
    const nav = adminPage.locator('nav.navbar')
  ...
  test('locale switcher is present on the Profile page', async ({ adminPage }) => {
    await adminPage.locator('nav.navbar .nav-actions button.nav-link', { hasText: 'Profile' }).click()
  ...
  test('switching to French updates nav labels', async ({ adminPage }) => {
    ...
    await expect(
      adminPage.locator('nav.navbar button.nav-link', { hasText: 'Paquets' })
