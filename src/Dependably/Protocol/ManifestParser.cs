...
public static class ManifestParser
{
    ...
    public static ManifestType Detect(string filename, string content)
    {
        ...
        if (lower.EndsWith("pipfile.lock", StringComparison.Ordinal)) return ManifestType.PipfileLock;
        if (lower.EndsWith("poetry.lock", StringComparison.Ordinal)) return ManifestType.PoetryLock;
        ...
        if (trimmed.StartsWith('{')) return DetectFromJsonContent(content);
        if (trimmed.Length > 0) return DetectFromTextContent(trimmed);
    ...
    {
        if (trimmed.Contains("[[package]]", StringComparison.Ordinal)
    ...
    private static List<ManifestEntry> ParsePipfileLock(string json)
    {
        ...
        using var doc = JsonDocument.Parse(json);
        var root = doc.RootElement;
        foreach (var section in (string[])["default", "develop"])
        {
    ...
    private static ManifestEntry? TryReadPipfileEntry(JsonProperty pkg)
    {
        ...
        if (!pkg.Value.TryGetProperty("version", out var ver) || ver.ValueKind != JsonValueKind.String) return null;
        var version = ver.GetString()!.Trim();
        if (!version.StartsWith("==", StringComparison.Ordinal)) return null;
    ...
    private static List<ManifestEntry> ParsePoetryLock(string toml)
    {
        ...
        if (model is not null && model.TryGetValue("package", out var pkgValue) && pkgValue is TomlTableArray packages)
        {
            foreach (var pkg in packages)
            {
                if (!pkg.TryGetValue("name", out var nameObj) || nameObj is not string name) continue;
                if (!pkg.TryGetValue("version", out var verObj) || verObj is not string version) continue;
                version = version.Trim();
                if (!PyPiArtifactValidator.IsPep440Version(version)) continue;
