...
///   5. OSV vulnerability score exceeds the tenant's <c>MaxOsvScoreTolerance</c>.
/// Records the corresponding <c>blocked_manual</c> / <c>blocked_deprecated</c> /
/// <c>blocked_release_age</c> / <c>blocked_vuln_score</c> activity row when a block fires
/// so the dashboard can surface why a download was denied.
/// </summary>
public sealed class BlockGateService
{
    ...
    {
        if (request.Deprecated is not null && IsAnyDeprecatedBlock(request.BlockDeprecatedMode))
            return await RecordDeprecatedBlockAsync(request, ct);
        return BlockDecision.Allowed;
    }

    // Single home for the deprecated-block side effects (meter + activity row) so the cache-hit
    // and first-fetch paths emit one consistent event shape.
    private async Task<BlockDecision> RecordDeprecatedBlockAsync(BlockGateRequest request, CancellationToken ct)
    {
        ...
        await _audit.LogActivityAsync(
            ...
            detail: $"{{\"deprecated\":{System.Text.Json.JsonSerializer.Serialize(request.Deprecated)}}}",
            sourceIp: request.SourceIp, ct: ct);
        return BlockDecision.Blocked;
    }
