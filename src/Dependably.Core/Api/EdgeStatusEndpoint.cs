...
public static class EdgeStatusEndpoint
{
    ...
    // Rounding precision so the JSON hit-rate value is stable and readable; exact ratios are
    // Grafana's job.
    private const int HitRateDecimalPlaces = 4;

    private static double HitRate(long hits, long misses)
    {
        long total = hits + misses;
        return total <= 0 ? 0d : Math.Round((double)hits / total, HitRateDecimalPlaces);
    }
