...
internal static class NpmPublishBodyParser
{
    ...
    /// </summary>
    // The four staging/parse stages (stream-to-file, locate the data token, decode, build the
    // redacted envelope) are kept in one method — each stage's own failure mode maps to a
    // distinct HTTP result, and splitting the stages would scatter that mapping across several
    // signatures without reducing the actual per-stage complexity that drives this method's length.
#pragma warning disable S138, S3776 // single streaming state machine, kept together for auditability
...
#pragma warning restore S138, S3776
...
#pragma warning restore S134, S3776
    ...
    // The forward byte-cursor scan (name → colon → quote → content) is one linear pass over the
    // hand-rolled cursor; splitting it into sub-steps would just relay the cursor position between
    // extra methods without reducing what the scan tracks.
#pragma warning disable S3776 // linear forward-cursor scan, kept together for auditability
...
#pragma warning restore S3776
    ...
    // The read/unescape/decode/write pipeline shares mutable carry state (escapeCarry, b64Carry)
    // across iterations; splitting a stage out would require threading that carry state through
    // an extra method boundary without reducing what the pipeline tracks.
#pragma warning disable S134, S3776 // streaming decode pipeline with cross-iteration carry state
...
#pragma warning restore S134, S3776
    ...
    // The carry-completion branch and the main scan loop share the same escapeCarry/dest-offset
    // state; separating them would require passing that state back and forth across an extra
    // method boundary without reducing what the scan tracks.
#pragma warning disable S3776 // carry-buffer unescape scan, kept together for auditability
...
#pragma warning restore S3776
    ...
    // The byte literals below are JSON escape codes and UTF-8 encoding masks, not arbitrary magic
    // numbers — naming each one adds indirection without adding clarity, so the whole hand-rolled
    // decode/encode block is kept together for auditability.
#pragma warning disable S109 // JSON escape codes / UTF-8 bit-twiddling constants, self-evident in context
    private static int DecodeEscape(ReadOnlySpan<byte> esc, byte[] dest, int offset)
    {
        ...
        {
            // '"', '\\', and '/' decode to themselves — already covered by the `_ => ind`
            // default below, so no explicit arm is needed for them.
            (byte)'b' => 0x08,
            (byte)'f' => 0x0C,
            (byte)'n' => 0x0A,
            (byte)'r' => 0x0D,
