...
public sealed class TokenAuthenticationHandler : AuthenticationHandler<TokenAuthenticationOptions>
{
    ...
    private static List<Claim> BuildClaims(TokenRecord token, string role)
    {
        ...
        {
            // "sub" matches JwtRegisteredClaimNames.Sub without pulling the IdentityModel
            // JOSE stack into this assembly — the ApiToken scheme issues no JWTs; it only
            // emits the same claim names the JWT session principals carry.
            new("sub", token.UserId ?? token.Id),
