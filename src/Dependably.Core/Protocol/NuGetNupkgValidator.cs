...
/// <c>version</c>. Used by the publish handler and the bulk-import controller.
/// Enforces: known nuspec namespace, id charset, version parseable by NuGet.Versioning,
/// mandatory description and authors fields.
/// </summary>
public static partial class NuGetNupkgValidator
{
    ...
    // All Microsoft nuspec namespace URIs across packaging schema versions.
    internal static readonly HashSet<string> KnownNuspecNamespaces = [
    ...
    /// <summary>
    /// Parses the .nupkg/.snupkg from a readable stream. The caller retains ownership of
    /// the stream (does not dispose it). Returns id+version on success, or a structured
    /// failure that callers can surface.
    /// </summary>
    public static (ValidationResult Result, string? Id, string? Version) ParseFromStream(Stream stream, bool isSymbol)
    {
        try
        {
    ...
    // Shared nuspec-validation logic. Both byte[] and Stream overloads open a ZipArchive
    // and delegate here so the rules stay in one place.
    private static (ValidationResult Result, string? Id, string? Version) ParseZip(ZipArchive zip, bool isSymbol)
    {
        if (isSymbol)
        {
