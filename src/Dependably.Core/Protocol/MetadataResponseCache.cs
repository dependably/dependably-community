...
public sealed class MetadataResponseCache
{
    ...
    /// <summary>
    /// Returns the stale response servable after a transient refresh failure, or null when none
    /// qualifies: the entry must exist, be a positive (2xx) entry, and still be within
    /// <see cref="MetadataCacheOptions.MaxStale"/> beyond its positive TTL. A negative (404) entry
    /// is never served stale — a transient failure refreshing a not-found should surface, not
    /// resurrect a stale miss as an answer.
    /// </summary>
    public UpstreamMetadataResponse? ShouldServeStale(string url)
    {
        ...
        return age <= _options.PositiveTtl + _options.MaxStale ? entry.Response : null;
    }
