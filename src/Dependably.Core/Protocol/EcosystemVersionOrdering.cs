...
public static partial class EcosystemVersionOrdering
{
    ...
    public static IReadOnlyList<string> OrderStableDescending(string ecosystem, IEnumerable<string> rawVersions) =>
        ecosystem switch
        {
            ...
            // versions are SemVer 2.0 as well — the registry rejects a release that is not — and
            // Hex mandates strict SemVer 2.0 for every release.
            "npm" or "cargo" or "terraform" or "hex" => OrderNpm(rawVersions),
    ...
    public static int? CountNewerStable(string ecosystem, IReadOnlyList<string>? stableVersionsDescending, string heldVersion)
    {
        return stableVersionsDescending is null || stableVersionsDescending.Count == 0
            ...
            : ecosystem switch
            {
                "npm" or "cargo" or "terraform" or "hex" => CountNewer<NpmSemver>(stableVersionsDescending, heldVersion, TryParseNpm, CompareNpm),
    ...
    public static int? Compare(string ecosystem, string left, string right) =>
        ecosystem switch
        {
            "npm" or "cargo" or "terraform" or "hex" => TryParseNpm(left, out var ln) && TryParseNpm(right, out var rn)
