...
#pragma warning disable SCS0018
        ...
        bool dryRun, CancellationToken cancellationToken)
    {
        ...
        foreach (var artefact in loaded)
        {
            ...
            try
            {
                var staged = new StagedFile(artefact.Filename, artefact.TempPath!, artefact.SizeBytes);
                outcome = await ImportDetectedAsync(
                    ctx, staged, detection, settings, dryRun, cancellationToken);
            }
            finally
            {
    ...
    // Groups the three pieces of per-file staging state (original filename, disk-staged
    // path, byte size) that ImportOneAsync/ImportDetectedAsync thread through unchanged.
    private sealed record StagedFile(string Filename, string TempPath, long SizeBytes);
    ...
    private async Task<object> ImportOneAsync(
        ImportContext ctx, StagedFile file, OrgSettings? settings,
        bool dryRun, CancellationToken ct)
    {
        ...
        {
            (ok, err) = EcosystemDetector.Detect(file.Filename, detectStream);
        }
        return ok is null
            ? Reject(file.Filename, err!.Code, err.Message, dryRun: dryRun)
            : await ImportDetectedAsync(ctx, file, ok, settings, dryRun, ct);
    }

    private async Task<object> ImportDetectedAsync(
        ImportContext ctx, StagedFile file,
        ...
        bool dryRun, CancellationToken ct)
    {
        ...
        var request = new PublishRequest
        {
            ...
            Purl = purl,
            ArtifactStagingPath = file.TempPath,
            ArtifactSizeBytes = file.SizeBytes,
            Origin = "uploaded",
            SizeCap = sizeCap,
