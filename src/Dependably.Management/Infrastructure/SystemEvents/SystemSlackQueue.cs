...
public sealed class SystemSlackQueue : BackgroundService, ISystemEventNotifier
{
    ...
    public SystemSlackQueue(
        ...
        : this(orgs, client, time, localizer, config, logger, backoffSchedule: null)
    {
    }
    ...
    /// interval a failing instance-wide Slack webhook is retried on.
    ///
    ...
    private async Task DeliverCoreAsync(SystemEventRecord record, CancellationToken ct)
    {
        ...
        for (int attempt = 0; attempt <= _backoffSchedule.Length; attempt++)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogDebug(ex,
                    ...
                    attempt + 1, record.Action, _backoffSchedule[attempt]);
                try
                {
        ...
        _logger.LogWarning(lastEx,
            ...
            _backoffSchedule.Length + 1, record.Action);
