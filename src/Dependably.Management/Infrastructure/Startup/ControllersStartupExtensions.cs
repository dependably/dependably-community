...
internal static class ControllersStartupExtensions
{
    internal static void AddDependablyControllers(this WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddControllers(options =>
            ...
            // The composition-root assembly (Dependably) hosts no controllers — they live in the
            // two class libraries. Controllers in a referenced class library are not auto-discovered,
            // so register each owning assembly as an application part explicitly.
            // Management controllers live here in Dependably.Management (anchored on this type);
            // protocol controllers live in Dependably.Core (anchored on a moved protocol controller).
            .AddApplicationPart(typeof(ManagementAssemblyMarker).Assembly)
