...
- **DataProtection key ring:** The standalone (no-Redis) path now always configures a durable DB-backed key ring via `DbXmlRepository` → `data_protection_keys` table. The key material is stored unencrypted in SQLite — the same posture as `jwt_secret` and `mfa_encryption_key` already in `instance_settings`. The HA (Redis) path is unchanged: `PersistKeysToStackExchangeRedis` handles multi-replica key sharing. A cross-cutting "encrypt all DB-resident secrets together, keyed from outside the DB" initiative would address this for all three secrets at once.
...
- Encrypting DB-resident secrets at rest: deferred as a cross-cutting initiative covering `jwt_secret`, `mfa_encryption_key`, and `data_protection_keys.xml` together, keyed from outside the DB.
