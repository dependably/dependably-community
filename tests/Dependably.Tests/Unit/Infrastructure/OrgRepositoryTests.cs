...
public sealed class OrgRepositoryTests : IClassFixture<InMemoryDbFixture>
{
    ...
    public async Task ListOrgsAsync_Aggregates_DontCartesianAmplify_AcrossUsersAndVersions()
    {
        ...
        var orgId = await OrgSeeder.InsertAsync(_fixture.Store, $"full-{Guid.NewGuid():N}");
        ...
        var pkgId = await PackageSeeder.InsertAsync(_fixture.Store, orgId, "npm", $"pkg-{Guid.NewGuid():N}");
    ...
    public async Task ListOrgsAsync_EmptyTenant_ReportsZeroAggregates_ViaCoalesce()
    {
        ...
        var orgId = await OrgSeeder.InsertAsync(_fixture.Store, $"empty-{Guid.NewGuid():N}");
    ...
    public async Task ListOrgsAsync_MixedPage_PopulatedAndEmptyTenants_BothCorrect()
    {
        ...
        var fullId  = await OrgSeeder.InsertAsync(_fixture.Store, $"mix-full-{Guid.NewGuid():N}");
        var emptyId = await OrgSeeder.InsertAsync(_fixture.Store, $"mix-empty-{Guid.NewGuid():N}");
        ...
        var pkgId = await PackageSeeder.InsertAsync(_fixture.Store, fullId, "npm", $"pkg-{Guid.NewGuid():N}");
    ...
    {
        var id = await OrgSeeder.InsertAsync(_fixture.Store, $"toggle-{Guid.NewGuid():N}");
    ...
    {
        var id = await OrgSeeder.InsertAsync(_fixture.Store, $"reject-{Guid.NewGuid():N}");
    ...
    {
        var id = await OrgSeeder.InsertAsync(_fixture.Store, $"del-{Guid.NewGuid():N}");
        await _repo.SoftDeleteOrgAsync(id);
    ...
    {
        var activeId    = await OrgSeeder.InsertAsync(_fixture.Store, $"cba-{Guid.NewGuid():N}");
        var suspendedId = await OrgSeeder.InsertAsync(_fixture.Store, $"cbs-{Guid.NewGuid():N}");
        var deletedId   = await OrgSeeder.InsertAsync(_fixture.Store, $"cbd-{Guid.NewGuid():N}");
        ...
        var before = await _repo.CountByStatusAsync();
        ...
        var aliceId     = await OrgSeeder.InsertAsync(_fixture.Store, $"alice-{Guid.NewGuid():N}");
        var bobId       = await OrgSeeder.InsertAsync(_fixture.Store, $"bob-{Guid.NewGuid():N}");
        await _repo.UpdateOrgStatusAsync(bobId, "suspended");
        var charlieId   = await OrgSeeder.InsertAsync(_fixture.Store, $"charlie-{Guid.NewGuid():N}");
        ...
        Assert.Equal(before.SoftDeleted + 1, after.SoftDeleted);
