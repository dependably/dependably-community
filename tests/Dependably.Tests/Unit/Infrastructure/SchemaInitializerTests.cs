...
public sealed class SchemaInitializerTests : IAsyncLifetime
{
    ...
    /// </summary>
    // Re-adds the seven retired per-org SMTP transport columns in the exact shape a pre-drop
    // release's additive migrations produce — CHECK included on email_smtp_security — so the drop
    // is exercised against a genuinely pre-drop database rather than a hand-simplified one. This is
    // also literally what an older slot does when it boots against a migrated database.
    private static async Task ResurrectRetiredSmtpColumnsAsync(DbConnection conn)
    {
        await conn.ExecuteAsync("ALTER TABLE alert_settings ADD COLUMN email_inherit_instance INTEGER NOT NULL DEFAULT 1");
        await conn.ExecuteAsync("ALTER TABLE alert_settings ADD COLUMN email_smtp_host TEXT");
        await conn.ExecuteAsync("ALTER TABLE alert_settings ADD COLUMN email_smtp_port INTEGER");
        ...
        await conn.ExecuteAsync("ALTER TABLE alert_settings ADD COLUMN email_smtp_username TEXT");
        await conn.ExecuteAsync("ALTER TABLE alert_settings ADD COLUMN email_smtp_password TEXT");
    ...
    /// <summary>
    /// The empirical answer to the question that decides this migration's shape: SQLite's
    /// documented refusal to DROP COLUMN for a column named in a CHECK applies to <em>table</em>-
    /// level CHECKs. <c>email_smtp_security</c> carries its own inline column-level CHECK, which
    /// SQLite drops together with the column — so a plain DROP COLUMN suffices and no
    /// recreate-table reshape is needed. If that were untrue against the pinned engine this test
    /// would throw rather than fail an assertion, which is the signal to switch shapes.
    /// </summary>
    [Fact]
    public async Task DropAlertSettingsRetiredSmtpColumns_RemovesTheCheckBearingColumn()
    {
        await NewInitializer(_db).InitializeAsync();

        await using (var setup = await _db.OpenAsync())
        {
            await ResurrectRetiredSmtpColumnsAsync(setup);
        }

        Assert.True(await AlertSettingsHasColumnAsync("email_smtp_security"),
            "setup failed: the CHECK-bearing column was not re-added");

        await NewInitializer(_db).InitializeAsync();

        Assert.False(await AlertSettingsHasColumnAsync("email_smtp_security"));
    ...
    /// <summary>
    /// The convergence property, and the reason the drop is not ledgered: a previous release's
    /// additive list still re-adds all seven columns, so an old slot booting against a migrated
    /// database resurrects them. The next boot of the current release must drop them AGAIN. A
    /// RunOnceAsync-ledgered drop fails this test, because the ledger already reads as done.
    /// </summary>
    [Fact]
    public async Task DropAlertSettingsRetiredSmtpColumns_RunsAgainAfterAnOlderSlotResurrectsThem()
    {
        await NewInitializer(_db).InitializeAsync();

        foreach (string column in RetiredSmtpColumns)
        {
            Assert.False(await AlertSettingsHasColumnAsync(column), $"{column} survived the first boot");
        }

        // An older release's slot boots against this database and re-adds its own columns.
        await using (var oldSlot = await _db.OpenAsync())
        {
            await ResurrectRetiredSmtpColumnsAsync(oldSlot);
        }

        // The current release boots again — no ledger reset, exactly as a rolling restart would.
        await NewInitializer(_db).InitializeAsync();

        foreach (string column in RetiredSmtpColumns)
        {
            Assert.False(await AlertSettingsHasColumnAsync(column),
                $"{column} was resurrected by an older slot and never dropped again — the drop is "
                + "ledgered, so the live schema has diverged from the schema file permanently");
        }
    }

    /// <summary>
    /// The drop removes columns and nothing else: every surviving value round-trips, and the PK,
    /// the orgs FK and the surviving vuln_min_severity CHECK are all still enforced afterwards.
    /// Proves this is a column removal rather than an accidental table-wide reshape.
    /// </summary>
    [Fact]
    public async Task DropAlertSettingsRetiredSmtpColumns_PreservesSurvivingColumnsAndConstraints()
    {
        await NewInitializer(_db).InitializeAsync();
            ...
            await setup.ExecuteAsync(
                ...
                INSERT INTO alert_settings
                    (org_id, quarantine_alerts_enabled, vuln_alerts_enabled, vuln_min_severity,
                     slack_enabled, email_enabled, email_recipients, email_inherit_instance,
                     email_smtp_host, email_smtp_password, email_last_status,
                     email_consecutive_failures)
                VALUES
                    ('o1', 0, 1, 'CRITICAL', 1, 1, 'ops@example.test', 0,
                     'smtp.example.test', 'enc:v1:ciphertext', 'failed', 3)
                """);
        }
        ...
        var (quarantine, vuln, severity, slack, emailEnabled, recipients, status, failures) =
            await verify.QuerySingleAsync<(long Quarantine, long Vuln, string Severity, long Slack,
                long EmailEnabled, string Recipients, string Status, long Failures)>(
                """
                SELECT quarantine_alerts_enabled, vuln_alerts_enabled, vuln_min_severity, slack_enabled,
                       email_enabled, email_recipients, email_last_status, email_consecutive_failures
                FROM alert_settings WHERE org_id = 'o1'
                """);

        Assert.Equal(0, quarantine);
        Assert.Equal(1, vuln);
        Assert.Equal("CRITICAL", severity);
        Assert.Equal(1, slack);
        Assert.Equal(1, emailEnabled);
        Assert.Equal("ops@example.test", recipients);
        Assert.Equal("failed", status);
        Assert.Equal(3, failures);

        // The surviving CHECK still bites.
        await Assert.ThrowsAnyAsync<Exception>(() => verify.ExecuteAsync(
            "UPDATE alert_settings SET vuln_min_severity = 'NOPE' WHERE org_id = 'o1'"));

        // The PK still bites.
        await Assert.ThrowsAnyAsync<Exception>(() => verify.ExecuteAsync(
            "INSERT INTO alert_settings (org_id) VALUES ('o1')"));
    }

    /// <summary>
    /// A fresh install has none of the seven columns, so every existence guard returns false and
    /// the pass is a no-op that repeats harmlessly on each boot.
    /// </summary>
    [Fact]
    public async Task DropAlertSettingsRetiredSmtpColumns_IsANoOpOnAFreshInstall()
    {
        await NewInitializer(_db).InitializeAsync();
    ...
    // The seven retired per-org SMTP transport columns, removed from every database by the
    // every-boot DropAlertSettingsRetiredSmtpColumnsAsync pass.
