...
/// <summary>
/// Regression coverage for <see cref="PyPiSimpleIndexHandler"/>'s local-only render path: a
...
/// on insert instead of surviving to <c>LocalTtl</c>.
/// </summary>
[Trait("Category", "Unit")]
public sealed class PyPiSimpleIndexHandlerReviewTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        await conn.ExecuteAsync("INSERT INTO orgs (id, slug) VALUES (@id, @slug)",
            new { id = _orgId, slug = "pypi-review-org" });
    }
    ...
    public async Task LocalOnly_ConcurrentProxySettingsInvalidateDuringRead_StaleRenderIsNotCached()
    {
        ...
        var cacheKey = new PyPiSimpleIndexKey(_orgId, "hosted-pkg");
        var cache = new InvalidateEpochDuringReadCache(
            new MemoryCache(new MemoryCacheOptions { SizeLimit = 32 * 1024 * 1024 }),
            MetadataCacheKeys.PyPiSimpleIndex, epochStore, cacheKey, _orgId);
        ...
        var result = await handler.PackageIndexAsync(http, _orgId, "hosted-pkg", CancellationToken.None);
        Assert.IsType<ContentResult>(result);

        // Old code (cache.Set with epochToken defaulting null — captured fresh at write time,
        // i.e. AFTER the Invalidate already ran) would bind to the NEW live epoch and survive
        // the flip. The fix captures the token before the read, so the write binds to the
        // already-retired token and is dropped/immediately evicted.
    ...
    public async Task LocalOnly_NoConcurrentInvalidate_RenderIsCached()
    {
        ...
        var cache = new RenderedResponseCache<PyPiSimpleIndexKey>(
            new MemoryCache(new MemoryCacheOptions { SizeLimit = 32 * 1024 * 1024 }),
            MetadataCacheKeys.PyPiSimpleIndex, epochStore);
        var handler = BuildHandler(cache);

        var http = BuildContext("host.example.test");
        Assert.IsType<ContentResult>(
            await handler.PackageIndexAsync(http, _orgId, "hosted-pkg", CancellationToken.None));
    ...
    private async Task SeedLocalPackageAsync()
    {
        ...
        await conn.ExecuteAsync("""
            ...
            VALUES (@id, @o, 'pypi', 'hosted-pkg', 'hosted-pkg', 0)
            """,
            new { id = "pkg-local", o = _orgId });
        await conn.ExecuteAsync("""
            INSERT INTO package_versions (id, package_id, version, purl, blob_key, origin, filename, created_at)
            VALUES (@id, 'pkg-local', '1.0.0', 'pkg:pypi/hosted-pkg@1.0.0', @bk, 'uploaded', 'hosted_pkg-1.0.0-py3-none-any.whl', @ts)
            """,
            new
            {
                id = "ver-local",
                bk = "registry/hosted-pkg-1.0.0.whl",
                ts = _clock.GetUtcNow().ToUtcIso(),
            });
    }

    private PyPiSimpleIndexHandler BuildHandler(RenderedResponseCache<PyPiSimpleIndexKey> cache)
    {
        ...
        var versionFiles = new PackageVersionFilesRepository(_db, _clock);
        ...
        return new PyPiSimpleIndexHandler(
            orgs, packages, versionFiles, tokens, vulns, inventory,
    ...
    private DefaultHttpContext BuildContext(string host)
    {
        ...
        http.Items[TenantContext.HttpItemsKey] = TenantContext.ForTenant(_orgId, "pypi-review-org");
    ...
    private sealed class InvalidateEpochDuringReadCache : RenderedResponseCache<PyPiSimpleIndexKey>
    {
        private readonly PyPiSimpleIndexKey _target;
        private readonly OrgCacheEpochStore _epochStore;
        private readonly string _orgId;
        private bool _fired;

        public InvalidateEpochDuringReadCache(
            IMemoryCache cache, Func<PyPiSimpleIndexKey, string> keyFormatter,
            OrgCacheEpochStore epochStore, PyPiSimpleIndexKey target, string orgId)
        ...
        public override bool TryGet(PyPiSimpleIndexKey key, out byte[]? value)
        {
